Monalisa Core version *0.15.1* is now available from:

  <https://download.monalisa.org/monalisa-0.15.1/>
...
A number of changes to the way Monalisa Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 leonards per kB (0.00001 MNL). This is to prepare for dropping the minimum transaction fee to 0.1 leonards per kB (0.0001 MNL) in the next release.
