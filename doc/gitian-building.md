...
*Setup instructions for a Gitian build of Monalisa Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Monalisa
...
- [Building Monalisa Core](#building-monalisa-core)
...
Clone the git repositories for monalisa and Gitian.
...
in the monalisa repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Monalisa Core
----------------

To build Monalisa Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the monalisa repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/monalisa/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/monalisa-project/monalisa
...
For example:
```bash
URL=https://github.com/thrasher-/monalisa.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit monalisa=${COMMIT} --url monalisa=${URL} ../monalisa/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit monalisa=${COMMIT} --url monalisa=${URL} ../monalisa/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit monalisa=${COMMIT} --url monalisa=${URL} ../monalisa/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the monalisa git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../monalisa/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/monalisa
SIGPATH=/some/root/path/monalisa-detached-sigs

./bin/gbuild --url monalisa=${BTCPATH},signature=${SIGPATH} ../monalisa/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/monalisa-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/monalisa-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/monalisa-osx-build.assert
```
...
[monalisa-project/gitian.sigs.ltc](https://github.com/monalisa-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
